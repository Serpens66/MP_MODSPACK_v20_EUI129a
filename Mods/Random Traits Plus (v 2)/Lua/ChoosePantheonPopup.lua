...
function RefreshList()
	...
	if (g_bPantheons > 0) then		
		...
		for i,v in ipairs(Game.GetAvailablePantheonBeliefs(Game.GetActivePlayer())) do
		...
		for i,v in ipairs(Game.GetAvailableReformationBeliefs(Game.GetActivePlayer())) do
			local belief = GameInfo.Beliefs[v];
			if(belief ~= nil) then
				table.insert(availableBeliefs, {
					ID = belief.ID,
