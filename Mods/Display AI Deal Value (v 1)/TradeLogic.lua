print("This is the modded TradeLogic from Display AI Deal Value mod")
----------------------------------------------------------------        
----------------------------------------------------------------        
include( "IconSupport" );
...
function DoUpdateButtons()
	...
	elseif (g_bPVPTrade == false and g_bTradeReview == false) then
        ...
        -- BEGIN Display AI Deal Value mod (also needs few changes in DiploTrade.xml)
		...
		if g_pThem.GetTotalValueToMeNormal~=nil and g_pThem.GetTotalValueToMe~=nil then -- if the correct DLL is active
            if(Teams[g_iUsTeam]:IsAtWar(g_iThemTeam)) then
                ...
                if(iCurrent == -9999 or iCurrent == -2) then -- in dll I think -9999 is returned, but in CP lua is asked for -2 , so just use both, to be sure.
                ...
                if(iCurrent == -9999 or iCurrent == -2) then
...
function ResetDisplay()
    ...
    if (not bResearchAgreementAllowed) then
		...
		elseif (g_pUsTeam:IsHasResearchAgreement(g_iThemTeam)) then
			...
			-- end
            local ourCash = g_pUs:GetGold() - iCost
            local theirCach = g_pThem:GetGold() - iCost
            if ourCash < 0 or theirCach < 0 then
                strDisabledTT = strDisabledTT .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_NO_AGREEMENT" )
                local spacer = " "
                if theirCach < 0 then
                    strDisabledTT = strDisabledTT .. spacer .. g_pThem:GetName() ..": " .. theirCach
                    spacer = ", "
                end
                if ourCash < 0 then
                    strDisabledTT = strDisabledTT .. spacer .. Locale.ConvertTextKey("TXT_KEY_YOU") ..": " .. ourCash
                end
            end
		end
...
function ChangeGoldAmount( string, control )
    ...
    -- BEGIN Display AI Deal Value mod (also needs few changes in DiploTrade.xml)
    DoUIDealChangedByHuman();
    -- END Display AI Deal Value mod (also needs few changes in DiploTrade.xml)
end
