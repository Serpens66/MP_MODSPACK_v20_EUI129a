...
local function DoUnitRangeStrikeAttack()
    ...
    else -- serp no range attack possible, eg there is no unit
        -- print("no range possible")
        EndUnitRangeStrikeAttack() -- serp: if we clicked at a plot without a unit, then we want to leave this interface!
        SetInterfaceMode( INTERFACEMODE_SELECTION ) -- game engine does nothing if already in selection mode
    end
    if unit~=nil and unit:IsOutOfAttacks() then -- serp
        -- print("outofattackes")
...
do
	...
	--]]
		-- if IsTouchScreenEnabled() then
			SetInterfaceMode( INTERFACEMODE_SELECTION ) -- serp, always go back to selection mode, otherwise we might get stuck in move_to mode
		-- end
		Events.DisplayMovementIndicator( false )
	...
	AssignInterfaceMode( INTERFACEMODE_SELECTION,
		{
            ...
            [MouseEvents.RButtonDown] = function() ShowUnitMovementRange( GetHeadSelectedUnit() ) return ShowUnitPath() end, -- serp show movement range instead while holding right click, like in vanilla
			[MouseEvents.RButtonUp] = function() ClearUnitMovementRange(); ClearStrikeRange(); EndMovement() return HideUnitPath() end, -- and hide again serp
		},
		function()
			-- BeginUnitRangeStrikeAttack( true ) -- serp
	...
	AssignInterfaceMode( InterfaceModeTypes.INTERFACEMODE_MOVE_TO,
		{
			[MouseEvents.LButtonUp] = function() ClearUnitMovementRange(); ClearStrikeRange(); HideUnitPath() return EndMovement() end, -- and hide again serp
			[MouseEvents.RButtonUp] = function() ClearUnitMovementRange(); ClearStrikeRange(); HideUnitPath() return EndMovement() end, -- and hide again serp
		},
		function()
			ShowUnitMovementRange( GetHeadSelectedUnit() )
			return ShowUnitPath()
...
AssignInterfaceMode( InterfaceModeTypes.INTERFACEMODE_CITY_RANGE_ATTACK,
	function()
		...
		if plot and not plot:IsFighting() then
			...
			if city and city:CanRangeStrikeAt( x, y, true, true ) then
				...
				-- SetInterfaceMode( INTERFACEMODE_SELECTION ) -- serp
			end
		end
        SetInterfaceMode( INTERFACEMODE_SELECTION ) -- serp, change the mode always!
        UI.ClearSelectedCities() -- serp, required for events to be processed properly
	...
	function()
		...
		if city and city:CanRangeStrike() then
            ...
            if city.GetBombardRange~=nil then -- serp compatible to variable bombard range mod from whoward
                iRange, iIndirect = city:GetBombardRange();
                return DisplayStrikeRange( city, city:Plot(), iRange, iIndirect == 1 )
            else
...
local function RefreshUnit()
	...
	if unit ~= g_lastUnit then
		...
		elseif interfaceMode == INTERFACEMODE_SELECTION then
			...
			-- ShowUnitMovementRange( GetHeadSelectedUnit() ) -- serp remove movement highlight on select
	...
	elseif unit then
		...
		-- end
		return
        -- return ShowUnitMovementRange( unit ) -- serp remove movement highlight on select
	end
end
...
ContextPtr:LoadNewContext("BarbarianScripts")
ContextPtr:LoadNewContext("BuildingsStorageYardAndStockpile")
ContextPtr:LoadNewContext("convenientstart")
ContextPtr:LoadNewContext("Stfu")
ContextPtr:LoadNewContext("NaturalWondersCustomMethods")
ContextPtr:LoadNewContext("GlobalCaptureGreatPeople")
...
ContextPtr:LoadNewContext("GlobalGAHappiness")
ContextPtr:LoadNewContext("GlobalSpiceIslands")
ContextPtr:LoadNewContext("MapsRandomGoodyHuts")
ContextPtr:LoadNewContext("AssignStartingPlots")
ContextPtr:LoadNewContext("Terra")
ContextPtr:LoadNewContext("ResetInfluence")
ContextPtr:LoadNewContext("PolicyFreeWarrior")
ContextPtr:LoadNewContext("PolicyRevealCapitals")
ContextPtr:LoadNewContext("QuestsCircumnavigate")
ContextPtr:LoadNewContext("QuestsNationalWonder")
ContextPtr:LoadNewContext("QuestsTourismContest")
ContextPtr:LoadNewContext("Randomize_Traits")
ContextPtr:LoadNewContext("LeaderTraitInfo")
ContextPtr:LoadNewContext("ExplorerGoodyBonus")
ContextPtr:LoadNewContext("SpainWonderBonusSpeedModifier")
ContextPtr:LoadNewContext("StratBuildRemove")
ContextPtr:LoadNewContext("TechSatellitesRevealCities")
ContextPtr:LoadNewContext("TradeRouteInfluence")
...
ContextPtr:LoadNewContext("InfluenceBoost")
ContextPtr:LoadNewContext("ColonistLua")
ContextPtr:LoadNewContext("wr_main")
ContextPtr:LoadNewContext("WorkMountains")
ContextPtr:LoadNewContext("SendGiftMessage")
