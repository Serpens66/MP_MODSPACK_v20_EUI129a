...
local g_ForceBuyOneTime = false -- serp force buy mode
...
local function SetupSelectionList( itemList, selectionIM, cityOwnerID, getUnitPortraitIcon )
	...
	for i = 1, #itemList do
		...
		if goldCost then
            ...
            instance.GoldButton:SetText( (cash>=goldCost and goldCost or "[COLOR_WARNING_TEXT]"..(goldCost).."[ENDCOLOR]") .. g_currencyIcon ) -- serp: removed the cost-cash stuff, always show the cost
        end
		...
		if faithCost then
            ...
            instance.FaithButton:SetText( (faith>=faithCost and faithCost or "[COLOR_WARNING_TEXT]"..(faithCost).."[ENDCOLOR]") .. "[ICON_PEACE]" ) -- serp: removed the cost-cash stuff, always show the cost
        end
			...
			if plot then
				...
				if city:CanBuyPlotAt( x, y, true ) then
					...
					if canBuy then
						...
						if plotCost==0 then
                            txt = "[COLOR_MAGENTA]" .. L("TXT_KEY_FREE").."[ENDCOLOR]" -- Serp, show "free" nistead of 0
                        else
                            txt = plotCost
                        end
						alpha = 1
                        ...
                        if plotCost==0 then
                            ...
                            txt = "[COLOR_WARNING_TEXT]"..(plotCost).."[ENDCOLOR]" -- Serp dont show the difference
						end
						alpha = 0.5
					end
...
function OnForceBuyMode()
	g_BuyPlotMode = true
    g_ForceBuyOneTime = true -- is set to false again after exiting city screen serp
end
LuaEvents.CityViewForceBuyPlotsMode.Add(OnForceBuyMode) -- compatibility to UI Expansion mod Serp and maybe everything else that is using vanilla CityViewForceBuyPlotsMode event
...
--print("enter city screen", GetHeadSelectedCity())
	...
	g_previousCity = nil
	if not g_ForceBuyOneTime and g_isResetCityPlotPurchase then -- serp force buy
		g_BuyPlotMode = false
...
--print("exit city screen")
	if UI.IsCityScreenUp() then
        ...
        g_ForceBuyOneTime = false -- serp, set force buy mode to false again
	end
end)
