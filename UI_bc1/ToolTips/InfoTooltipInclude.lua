...
if Game then
    ...
    local function GetLeaderName( player )
        ...
        elseif player:IsHuman() then -- Game.IsGameMultiPlayer()
            ...
            if n and n ~= "" then
                local leadername = L( (GameInfo.Leaders[ player:GetLeaderType() ] or {}).Description ) -- serp, added real leadername, so you don see only the nickname
                local icons = ""
                if leadername and leadername~="" then
                    for icon in string.gmatch(leadername, "%[.-%]") do 
                        icons = icons..icon
                    end
                end
                return player:GetNickName()..icons -- if active, show the icons from randomtraits mod
            end
        end
