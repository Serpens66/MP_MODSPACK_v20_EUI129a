...
--	Controls.StatusIconBG:SetToolTipString(strStatusTT)
    ...
    ---------------------------------
    -- You get info in popup about protected by, added by Serp

    -- Protected by anyone?
    local protected_tooltip = Locale.ConvertTextKey("TXT_KEY_POP_CSTATE_PROTECTED_BY_TT")
	if protected_tooltip~="TXT_KEY_POP_CSTATE_PROTECTED_BY_TT" then
        Controls.ProtectLabel:SetHide( false )
        Controls.ProtectInfo:SetHide( false )
    ...
    else -- string does not exist (currently within city state gifts mod, maybe add it to EUI strings...)
        Controls.ProtectLabel:SetHide( true )
        Controls.ProtectInfo:SetHide( true )
    end
	...
	-- Peace
		...
		-- Gifts  , changed by SERP, to make compatible to city state gifts mod
		if m_lastAction == kiGreet then
            
            if m_PopupInfo.Text~=nil and m_PopupInfo.Text~="" then -- then the city states gift mod is active
                ...
                if (m_PopupInfo.Data2 == 0) then
                  ...
                  if (m_PopupInfo.Text == "UNIT") then
                    strGiftString = Locale.ConvertTextKey(strGiftTxtKey, GameInfo.Units[m_PopupInfo.Data2].Description, personalityInfo.title, m_PopupInfo.Data3)
                  else
                    strGiftString = Locale.ConvertTextKey(strGiftTxtKey, m_PopupInfo.Data2, personalityInfo.title, m_PopupInfo.Data3)
                  end
                end

                local strSpeakAgainString = Locale.ConvertTextKey("TXT_KEY_MINOR_SPEAK_AGAIN", strShortDescKey);
                strText = Locale.ConvertTextKey("TXT_KEY_CITY_STATE_MEETING", strShortDescKey, strGiftString, strSpeakAgainString);
                --Events.GameplayAlertMessage("Ihr habt den Stadtstaat [COLOR_POSITIVE_TEXT]" .. Locale.ConvertTextKey(strShortDescKey) .. "[ENDCOLOR] getroffen! [NEWLINE]" .. strGiftString);
            else -- normal game without citystate gifts mod
...
ContextPtr:SetShowHideHandler( function( bIsHide, bInitState )
	if not bInitState then
		...
		if not bIsHide then
			-- if m_PopupInfo.Type ~= ButtonPopupTypes.BUTTONPOPUP_CITY_STATE_GREETING then  -- if condition by serp, because on greeting, ondisplay() is called twice
                Events.SerialEventGameDataDirty.Add( OnDisplay )
                --print("dirty event added")
            -- end
